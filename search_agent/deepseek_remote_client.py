"""
DeepSeek V4 Pro client with balance logging and cache hit tracking.
Based on openai_remote_client.py but without vLLM-specific extra_body.

Usage:
  python search_agent/deepseek_remote_client.py \
    --searcher-type bm25 --index-path indexes/original/bm25 \
    --output-dir runs/original/deepseek_v4_pro_bm25 --num-threads 8

  MAX_QUERIES=8 python search_agent/deepseek_remote_client.py \
    --searcher-type faiss --index-path indexes/crosslingual/qwen3-embedding-8b/corpus.pkl \
    --model-name Qwen/Qwen3-Embedding-8B --normalize \
    --dataset-name data/crosslingual/crosslingual_corpus_tevatron.jsonl \
    --output-dir runs/crosslingual/deepseek_v4_pro_qwen3-8b --num-threads 8
"""
...
def _check_balance(api_key, base_url):
    try:
        url = base_url.rstrip("/") + "/user/balance"
        r = _requests.get(url, headers={"Authorization": f"Bearer {api_key}"}, timeout=10)
...
def _log_balance(balance_data, label, log_path, output_dir):
    if not balance_data:
        return
    ts = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
    lines = [f"[{label}] {ts}  output_dir={output_dir}"]
    for info in balance_data.get("balance_infos", []):
        lines.append(f"  {info['currency']}: total={info['total_balance']}, "
                      f"granted={info['granted_balance']}, topped_up={info['topped_up_balance']}")
    text = "\n".join(lines)
    print(text)
...
def run_conversation_with_tools(client, model, messages, tools, tool_handler, max_iterations=100, verbose=False):
    tool_usage = {}
    iteration = 1
    ...
    while iteration <= max_iterations:
        ...
        if usage and verbose:
            ...
            print(f"  [turn {iteration}] prompt={usage.prompt_tokens} hit={cache_hit} miss={cache_miss} out={usage.completion_tokens}")
...
def _persist_response(out_dir, model, messages, tool_usage, status, query_id=None):
...
def _process_tsv_dataset(tsv_path, client, args, tool_handler):
    ...
    def _handle_single_query(qid, qtext, pbar=None):
        messages = [{"role": "user", "content": format_query(qtext, args.query_template)}]

        try:
            messages, tool_usage, status = run_conversation_with_tools(
                client, args.model, messages, tools, tool_handler, args.max_iterations, args.verbose
            )
            if status == "completed":
                with completed_lock:
                    completed_count[0] += 1
            ...
            _persist_response(out_dir, args.model, messages, tool_usage, status, query_id=qid)
...
def main():
    parser = argparse.ArgumentParser(
        description="DeepSeek V4 Pro client with balance logging and cache tracking.",
    ...
    parser.add_argument("--output-dir", default="runs/deepseek_v4_pro")
    ...
    balance_log = os.path.join(args.output_dir, "balance_log.txt")
    os.makedirs(args.output_dir, exist_ok=True)
    bal_before = _check_balance(args.api_key, args.model_url)
    _log_balance(bal_before, "BEFORE", balance_log, args.output_dir)
    ...
    if isinstance(args.query, str) and args.query.strip().lower().endswith(".tsv"):
        ...
        if potential_path.is_file():
            ...
            bal_after = _check_balance(args.api_key, args.model_url)
            _log_balance(bal_after, "AFTER", balance_log, args.output_dir)
            return
