"""
GPT-OSS-20B search agent client.

Uses vLLM Responses API with local search tool handler
(same pattern as oss_client.py).

Usage:
  # Start vLLM server with gpt-oss-20b:
  vllm serve openai/gpt-oss-20b --port 8000

  # Then run the agent (with local searcher, no separate MCP server needed):
  python search_agent/gpt_oss_client.py \
    --model openai/gpt-oss-20b \
    --model-url http://localhost:8000/v1 \
    --searcher-type bm25 \
    --index-path indexes/crosslingual/bm25 \
    --output-dir runs/crosslingual/gpt_oss_20b \
...
def _persist_response(
    ...
    *,
    ...
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(normalized_record, f, indent=2, default=str, ensure_ascii=False)
...
def _process_tsv_dataset(
    ...
    def _handle_single_query(qid: str, qtext: str, pbar=None):
        ...
        initial_request = {
            ...
            "input": [
                {"role": "user", "content": format_query(qtext, args.query_template, language=language)}
            ],
...
def main():
    parser = argparse.ArgumentParser(
        description="GPT-OSS-20B search agent with local retrieval tool.",
    ...
    parser.add_argument(
        "--output-dir",
        default="runs/crosslingual/gpt_oss_20b",
    ...
    parser.add_argument(
        ...
        help="Reasoning effort (omit to use model default)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        required=True,
        help=f"Type of searcher: {', '.join(SearcherType.get_choices())}",
    )
    parser.add_argument("--hf-token", type=str, help="Hugging Face token")
    parser.add_argument("--hf-home", type=str, help="Hugging Face home directory")
    parser.add_argument(
        "--snippet-max-tokens",
        type=int,
        default=512,
    ...
    parser.add_argument(
        "--k", type=int, default=5, help="Top-k search results per query"
    )
    parser.add_argument(
        "--get-document",
        action="store_true",
        help="Enable get_document tool",
    )
    parser.add_argument(
        "--lang-map",
        type=str,
        default=None,
        help="Path to query language map JSON (e.g. data/crosslingual/query_lang_assignment.json)",
    )
