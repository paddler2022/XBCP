"""
Remote OpenAI SDK client for Qwen3.6 / any model behind an OpenAI-compatible endpoint.
Uses Chat Completions API (not Responses API) for maximum compatibility.

Usage:
  python search_agent/openai_remote_client.py \
    --model Qwen/Qwen3.6-35B-A3B \
    --model-url https://your-qwen36-endpoint/v1 \
    --searcher-type faiss \
    ...
    --output-dir runs/crosslingual/qwen36_35b_qwen3-8b \
    --num-threads 10
"""
...
def run_conversation_with_tools(client, model, messages, tools, tool_handler, max_iterations=100, verbose=False):
    """Multi-turn chat completions with tool calling."""
    tool_usage = {}
    iteration = 1
...
def _persist_response(out_dir, model, messages, tool_usage, status, query_id=None):
    ...
    for msg in messages:
        ...
        # Record reasoning (thinking) if present
...
def main():
    parser = argparse.ArgumentParser(
        description="Remote OpenAI SDK client with local retrieval tool.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--query", default="topics-qrels/queries.tsv")
    parser.add_argument("--model", default="Qwen/Qwen3.6-35B-A3B", help="Model name on remote endpoint")
    parser.add_argument("--model-url", default="https://your-qwen36-endpoint/v1", help="Remote endpoint URL")
    parser.add_argument("--api-key", default=os.environ.get("OPENAI_API_KEY", "EMPTY"), help="API key")
    parser.add_argument("--max-tokens", type=int, default=10000)
    parser.add_argument("--output-dir", default="runs/crosslingual/qwen36_35b_remote")
